...
- **Recognize approval intent; persist the gate; never loop.** Treat clear approval replies — "yes", "approve", "approved", "proceed", "go ahead", "do it", "write the plan", "create the plan", and localized equivalents (e.g. Korean "네", "응", "진행", "ㄱㄱ", "작성해") — as approval; do not demand one exact magic phrase. When you present the brief, record the pending gate in `.omo/drafts/<slug>.md` (exact pending action + target `.omo/plans/<slug>.md` path) so the next turn reads that durable state instead of re-running exploration or restating the whole plan. If the reply is still not approval, ask ONE concise approval question — never repeat the full blocked-gate message. A reply that amends scope updates the brief, not the plan.
