...
When a higher-level research skill (especially `ultraresearch`) asks for blocked-source evidence, return an `INSANE_SOURCE_RECORD` instead of only prose. Include `url`, `final_url`, `title`, `access_method`, `waf_profile`, `verdict`, `selector_proof`, `fetched_at`, `trace_summary`, `source_quality_hint`, and extracted text or a pointer to it. This keeps WAF-bypassed sources eligible for the research source registry, citation ledger, and later claim verification.
